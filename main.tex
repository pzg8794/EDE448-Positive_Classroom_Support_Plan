...
\hypersetup{colorlinks=true,linkcolor=DeepNavy,urlcolor=DeepNavy,citecolor=DeepNavy}
...
\reviewsection{My Prevent--Teach--Reinforce Classroom Plan}

PTR gives this classroom plan an accountable structure \citep{dunlap2019ptr}. Positive self-talk is not isolated from classroom architecture. Adults must prevent unnecessary barriers, teach a usable reflection and communication process, and reinforce the process by making the learner's chosen support effective.

\small
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{>{\RaggedRight\arraybackslash}p{1.22in} >{\RaggedRight\arraybackslash}p{2.58in} >{\RaggedRight\arraybackslash}p{2.82in}}
\tableheaderthree{PTR Component}{Teacher and Environment Commitments}{Student Access and Agency}
\endfirsthead
\tableheaderthree{PTR Component}{Teacher and Environment Commitments}{Student Access and Agency}
\endhead
Prevent &
Make the lesson route visible; model the first step; reduce unnecessary language and speed pressure; provide sensory, movement, technology, and communication supports before distress; protect private participation. &
Students can see what is required, choose among meaningful routes, keep an effective familiar tool, request more time or a break, observe first, pass, and re-enter without shame. \\
\rowcolor{SoftGray}
Teach &
Model the difference among a thought, feeling, fact, need, and action. Use examples from ordinary academic errors. Teach learners to locate evidence, select believable words, and connect those words to a support or next move. &
Students may create, select, type, point to, speak, sign, or internally use a phrase. They may reject an adult's wording and propose language that fits their experience. \\
Reinforce &
Make the communication effective; recognize observable contribution with consent; give specific private or public feedback according to preference; notice quiet and nontraditional participation; revise inaccessible routines. &
Self-advocacy leads to an actual response. Recognition strengthens communication, contribution, repair, persistence, and belonging rather than eye contact, stillness, speed, or cheerful compliance. \\
\end{longtable}
\normalsize

The Edutopia classroom-management session describes structure, predictability, relationships, transitions, and instructional clarity as central to classroom orchestration \citep{edutopiaClassroom}. That framing helps me locate many concerns in lesson design. It also applies to teachers: I need reflective language that lets me say, ``I do not understand this yet; I can gather evidence and ask the student,'' instead of defending the first interpretation because I am supposed to have every answer.
...
\rowcolor{SoftGray}
An error or unexpected result &
``This result is evidence. I can test one change.'' &
Debugging guide, comparison example, material reset, peer consultation, or another trial. \\
...
\color{white}\bfseries Prevent and Access &
...
``I noticed\ldots,'' ``I tested\ldots,'' ``I can try\ldots,'' or ``I need\ldots'' &
...
If students repeat the desired words while losing communication, regulation, trust, or access, the plan has failed. If a phrase does not help, the team should examine the language, the task, the support, the hypothesis, and adult implementation before deciding that the learner is resistant. PTR requires continued data-based revision, not loyalty to an intervention because adults designed it \citep{dunlap2019ptr}.
